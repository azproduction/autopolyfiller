...
function AutoPolyFillerCli(process, done) {
    ...
    this.run(this.process.stdin.isTTY, done || function () {});
}
...
AutoPolyFillerCli.prototype = {
    ...
    addFile: function (fileContents, filePath) {
        try {
            ...
            console.error('Error while adding file from %s: %s', 'filePath', e);
            this.process.exit(1);
        }
    },
    ...
    writePolyFills: function (done) {
        ...
        this.getOutput().write(this.polyfiller.toString(), 'utf8', done);
    },

    run: function (isTTY, done) {
        if (isTTY) {
